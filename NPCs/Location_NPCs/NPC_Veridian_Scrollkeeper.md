---
name: Veridian Scrollkeeper
type: NPC
status: Active
version: "1.0.0"
tags: [npc, location-npc, agastia, scholar-quarter, artifact-expert, codex-knowledge, session2]
location: Agastia Scholar Quarter - Archive of Lost Histories
occupation: Artifact Scholar / Archivist
race: Half-Elf
gender: Male
threat_level: None
related_entities: [Codex of Peace, Temporal Bloodline, Garrek, Manny]
---

# Veridian Scrollkeeper

## Player Summary

**Veridian Scrollkeeper** is an elderly half-elf scholar who has dedicated his life to studying ancient civilizations and lost artifacts. He works at the Archive of Lost Histories in Agastia's Scholar Quarter, surrounded by dusty tomes and fragmented texts. Known for his enthusiasm when discussing ancient lore, Veridian loves sharing knowledge with anyone genuinely interested.

### Basic Information
- **Race:** Half-Elf
- **Gender:** Male
- **Age:** Elderly (appears ~80-90, but half-elf aging means could be 150+)
- **Occupation:** Artifact Scholar / Archivist
- **Location:** Archive of Lost Histories, Scholar Quarter, Agastia
- **Personality:** Enthusiastic, generous with knowledge, slightly absent-minded

### Appearance
- Thin frame, slightly stooped from years bent over books
- Silver-white hair pulled back in a simple tail
- Uses ornate walking stick (gift from former student)
- Reading spectacles perpetually perched on nose
- Ink stains on fingers from decades of research
- Simple scholar's robes in faded blue

### Demeanor
- Lights up when discussing ancient artifacts
- Speaks rapidly when excited about a topic
- Sometimes trails off mid-sentence when remembering a reference
- Will retrieve books mid-conversation to verify details
- Patient with genuine seekers of knowledge
- Impatient with those who waste his time

### Known Activities
- Works daily at Archive of Lost Histories
- Researches pre-Merit Council artifacts and civilizations
- Advises scholars seeking historical information
- Has spent 40+ years studying peace-binding artifacts
- Known in academic circles as leading expert on ancient items
- Maintains extensive personal notes and cross-references

### Personality & Behavior
- Enthusiastic and generous with knowledge
- Becomes animated when discussing topics of interest
- Absent-minded about practical matters
- Patient teacher to genuine students
- Dismissive of treasure hunters and grave robbers
- Values knowledge preservation over profit

## Knowledge Base

### Area of Expertise
**Ancient Civilizations & Artifacts:**
- Pre-Merit Council history (specialty)
- Lost magical items and their properties
- Ancient defensive magic and binding rituals
- Temporal anomalies and time-related artifacts
- Underdark civilizations and their remnants

### The Dominion Evolution Codex

**What Veridian Knows:**
- Studied legends of "peace-binding artifacts" for 40+ years
- Found references to a "Seven-Page Codex" in fragmentary texts
- **Ancient Text Reference:** "Book of Seven Metals, bound to stone eternal"
- **Properties:** Said to enforce peace itself - prevents violence in its presence
- **Location Clues:** Hidden in sanctuary where "time flows strangely"
- **Healing Waters:** Old explorer journals mention miraculous healing
- **Guardians:** Temporal Bloodline Tribe mentioned as keepers
- **Warning:** One text warned watchers must be "pure of heart and clear of purpose"

**Sources:**
- Fragment from pre-Council tome (Archive reference: LC-4072)
- Explorer journal dated 180 years ago (partial, water-damaged)
- Oral traditions collected from Underdark traders
- Cross-referenced symbols matching Codex descriptions

### Other Relevant Knowledge
- Familiar with Garrek's reputation (famous explorer, presumed dead 20 years ago)
- Knows general Underdark geography (academic knowledge)
- Can identify temporal magic effects when described
- Understands ancient binding rituals and their purposes

## Relationships

### Colleagues
- **Mirella Stonemark:** Philosophical disagreement (he preserves, she profits); professional respect but different values

### Employers
- **Archive of Lost Histories:** Employed as senior researcher and archivist

### Potential Connections
- **Manny:** Will become mentor figure if Manny shares discoveries
- **Professor Zero:** May know of him through academic circles (not confirmed)

## How to Find Him

### Location
**Archive of Lost Histories** (Scholar Quarter, Tier 3)
- Small, specialized library focusing on pre-Council artifacts
- Not widely known outside academic circles
- Open during standard hours (8am - 6pm, every day)
- Veridian practically lives there

### Methods
**DC 12 Investigation in Scholar Quarter:**
- Ask at any library about "artifact experts"
- Scholars recognize the name when artifacts are mentioned
- "Oh, you want Veridian - he's at the Archive of Lost Histories"

**Torvin's Recommendation:**
- If party asks Torvin about artifact scholars
- "Veridian Scrollkeeper - old as dirt, knows twice as much"
- Provides Archive location

**Professor Zero Connection:**
- Professor Zero may have mentioned Veridian in passing
- Academic circles all know each other

## Interaction with Manny

### Initial Meeting
When Manny asks about the Codex:

> Veridian's eyes light up behind his spectacles. He sets down his current book with a soft thump.
>
> "Ah, the Dominion Evolution Codex! A fascinating legend. Seven metal pages, bound to stone, supposedly capable of enforcing peace itself."
>
> He starts pulling books from shelves even as he talks.
>
> "The old texts say it was placed in a sanctuary where time flows... strangely. Healing waters, if the stories are true. Look for the Temporal Bloodline—they were the guardians, last I heard."

### Follow-Up Questions
**"Where is this sanctuary?"**
- "The texts don't give specifics - ancient authors loved their vagueness."
- "Underdark, certainly. References to stone and water, echoing chambers."
- "One fragment mentioned 'where tears of stone weep endlessly' - a waterfall, perhaps?"

**"Who are the Temporal Bloodline?"**
- "Tribe with innate temporal magic - very rare."
- "Can sense distortions in time, sometimes manipulate it minutely."
- "Reclusive. Haven't been seen on surface in... oh, century or more?"
- "If anyone knows the Codex's exact location, it's them."

**"How do I find them?"**
- "That's the trick, isn't it? They find you, usually."
- "Stories say they appear near temporal anomalies."
- "Places where time flows wrongly - faster, slower, backwards."
- "If you're searching for the Codex, they may seek you out."

### Payment
**FREE - No charge**
- Veridian loves sharing knowledge
- Seeing a young person interested in ancient lore delights him
- May gift Manny a reference copy of relevant text
- "Just promise to tell me what you find!"

## DM Notes

### Veridian's Role
- **Information Source:** Provides lore-based clues to Codex location
- **Academic Connection:** Can validate/expand on journal fragments
- **Future Resource:** Party can return for other research needs
- **Enthusiasm Balance:** Excited to help but doesn't have exact answers

### What He Doesn't Know
- Exact location of Garrek's Falls (never been there)
- That Garrek is dead (thinks he's still exploring somewhere)
- Specific route to Underdark sanctuary
- Dangers of the healing waters (addictive property)
- Codex removal mechanism or triggers

### Secrets
- **Hidden Detail:** Veridian once tried to find Temporal Bloodline himself
  - Failed, gave up after 10 years
  - Still dreams of seeing the Codex before he dies
- **Academic Rivalry:** Has mild disagreement with Mirella Stonemark
  - She sells artifacts, he preserves them
  - Professional respect, philosophical differences

### If Manny Shares Discoveries
- **Thrilled** if Manny reports back with findings
- May help decode journal pages
- Can research specific questions between sessions
- Potential mentor figure for Manny's scholarly interests

### Connection to Session 2
- One of two scholars providing Codex leads to Manny
- Complements Mirella's pragmatic approach with academic enthusiasm
- FREE information balances Mirella's paid intel
- Can be visited during Agastia exploration

### Future Hooks
- May ask party to retrieve specific artifacts for Archive
- Could provide research on other mysteries (Josh's markings, etc.)
- Academic contact in Agastia for party
- Potential patron for artifact-recovery quests
