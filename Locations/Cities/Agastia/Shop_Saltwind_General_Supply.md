---
name: Saltwind General Supply
type: Shop
subtype: Sailor & Ship Provisions
district: Dock District
city: Agastia
tier: 6
status: Active
version: "1.0.0"
tags: [commerce, dock-district, sailor, ship-supplies, maritime]
---

# Saltwind General Supply

## Overview

**Type & Specialty:** Sailor provisions and ship supplies - rope, canvas, tools, preserved food, navigation equipment, and everything needed for maritime work

**Location:** Dock District (Tier 6), Pier Row, three buildings down from Warehouse 7

**Clientele:** Sailors, dockworkers, fishermen, ship captains, caravan guards (rope and canvas), anyone needing maritime supplies

**Reputation:** "Olric's prices are fair and his rope don't break." Reliable quality for working sailors. Not fancy, but honest goods that last.

**Atmosphere:** Smells of tar, salt, and canvas. Coils of rope everywhere. Sails and tarps hang from rafters. Sound of dock work filters in - creaking ships, shouting workers, seabirds. Practical, no-frills, organized chaos.

## Proprietor

**Olric Saltwind** - Half-elf male, early 60s, former ship's quartermaster (retired)

**Personality:**
- Straight-talking pragmatist - no patience for haggling or nonsense
- Generous to those in genuine need, stubborn against cheats
- Deeply worried about dock district corruption but feels powerless

**Appearance:** Weather-beaten face, salt-and-pepper hair in sailor's queue, muscular build going soft with age. Always wears faded blue coat (old ship uniform). Walks with slight limp (old injury).

**Motivation:** Retired from sea after leg injury. Opened shop to serve honest sailors - the people he worked with for 40 years. Wants to protect his community from corruption creeping into the docks.

**Quest Hook - The Inspection Harassment:**
- **Need:** "Inspector Geist" has been targeting Olric's shop for repeated "inspections." Every time, Geist finds minor violations (real or fabricated) and demands "fines" - really bribes. Olric can't afford this much longer.
- **Problem:** Geist has threatened to confiscate Olric's entire inventory for "safety violations" if he doesn't pay 50 gp immediately. Olric knows it's extortion but can't prove it. He needs evidence of Geist's corruption or a way to stop the harassment.
- **Reward:** In exchange for stopping Geist's harassment (with proof or permanent solution):
  - 15% permanent discount on all maritime supplies
  - Access to "sailor's network" information (what ships have seen, where they've been, rumors from other ports)
  - Quality rope and canvas guaranteed - Olric personally tests every coil
  - Introduction to honest dock captains (potential ship passage, information, allies)
  - Hidden storage space in shop for party's gear (secure, no questions asked)
- **Difficulty:** Moderate to High - Directly connected to Kyle's Geist investigation. Gathering evidence on Geist is challenging and potentially dangerous.

## Day-to-Day Operations

**Hours:**
- Dawn to dusk (5am-7pm): Sailors work early, Olric opens early
- Busiest: Early morning (ships departing) and late afternoon (ships arriving)

**Typical Day:**
- Pre-dawn: Olric preps shop, checks inventory, tests rope quality
- Morning rush: Sailors buying supplies before departures, ship captains placing bulk orders
- Midday: Deliveries, repairs, custom orders (sail mending, rope splicing)
- Afternoon: Arriving ships' crews replacing used supplies
- Evening: Close shop, prepare next day's orders, worry about Geist

**Regular Customers:**
- **Ship crews:** Replacing used supplies, stocking for voyages
- **Fishermen:** Nets, hooks, line, preservation supplies
- **Dockworkers:** Tools, work gloves, heavy rope
- **Caravan guards:** Rope, canvas tarps, waterproof containers
- **Ship captains:** Bulk orders, custom work, navigation equipment

**Real Uses:**
- **Maritime supplies:** Essential equipment for anyone working on water or with ships
- **Rope quality:** Olric's rope is tested - climbers and delvers trust it
- **Information:** Sailors gossip - Olric hears news from distant ports, sea monster sightings, political rumors
- **Safe storage:** Hidden compartment for party's gear (not advertised, but offered to allies)
- **Geist investigation:** Olric is victim and can provide testimony/evidence about protection racket
- **Ship passage:** Olric knows honest captains who'll give fair prices for passage

## Services & Inventory

**Primary Services:**

**Rope & Canvas:**
- Rope (by foot): 1 sp per 10 feet (hempen), 5 sp per 10 feet (silk)
- Canvas (by yard): 1 sp per square yard
- Custom rope splicing: 2 sp per hour
- Sail mending: 5 sp per square yard
- Waterproofing service: 1 gp per large item

**Tools & Equipment:**
- Standard maritime tools at PHB prices
- Tool repair/sharpening: 5 cp per tool

**Provisions:**
- Bulk dried/salted food for ship provisioning
- Preservation supplies (salt, barrels, wax)

**Navigation:**
- Maps, compasses, navigation tools (limited selection)

**Inventory:**

**Toggle: Common Items (Always Available)**
- **Rope (hempen):** 1 sp per 10 ft - Olric tests every coil personally
- **Rope (silk):** 5 sp per 10 ft - for climbers, lighter and stronger
- **Canvas (sail-grade):** 1 sp per sq yard - waterproof treatment available
- **Tar (waterproofing):** 5 sp per pot
- **Fishing net:** 1 gp - various sizes
- **Fish hooks:** 1 cp each, 1 sp per dozen
- **Fishing line:** 1 sp per 100 ft
- **Ship's anchor (small):** 5 gp
- **Oars:** 2 gp per pair
- **Barrel (waterproof):** 2 gp - food-grade
- **Block and tackle:** 1 gp
- **Marlinspike:** 5 sp - rope working tool
- **Work gloves (heavy):** 5 sp - canvas and leather, protects hands
- **Salt (preservation):** 5 sp per pound

**Toggle: Uncommon Items (Sometimes Available)**
- **Masterwork rope:** 2x cost - Olric's personally crafted, advantage on break checks
- **Waterproof map case:** 5 gp - brass fittings, completely sealed
- **Quality compass:** 25 gp - more accurate than standard
- **Storm lantern:** 10 gp - stays lit in heavy wind/rain
- **Climbing harness:** 15 gp - Olric's design, distributes weight safely
- **Signal flags (set):** 10 gp - maritime communication
- **Spyglass (simple):** 50 gp - used but functional

**Toggle: Special Items (Quest Rewards or High Price)**
- **Dwarven rope (100 ft):** 150 gp - incredibly strong, doesn't fray, advantage on all checks
- **Captain's logbook:** 25 gp - waterproof, includes navigation tables and blank pages
- **Masterwork navigation tools:** 100 gp - advantage on Navigation checks at sea
- **Storm survival kit:** 75 gp - everything needed to survive shipwreck (waterproof container, rations, signal mirror, flare, fishing kit, water purification tablets)
- **Sea charts (regional):** 50-100 gp - detailed maritime maps showing currents, hazards, ports

**Pricing:** Fair market rates. Olric doesn't price-gouge even though he could (monopoly on quality rope in docks).

## Connections

**District Integration:**
- Essential service for entire Dock District - sailors depend on Olric
- Competes with cheaper (lower quality) suppliers, but reputation keeps him in business
- Informal hub for sailor gossip and information exchange
- Victim of Geist's protection racket (like many dock businesses)

**Related NPCs:**
- **Mira Saltwind:** Niece - runs Saltwind Imports in Merchant District (different business, same family)
- **Geist:** Primary antagonist - extorting Olric's shop for "inspection fees"
- **Honest ship captains:** Olric's contacts from decades at sea, trust him completely
- **Harren (Dock Worker):** Regular customer, buys work gloves and rope, shares dock gossip

**Faction Ties:**
- **Sailor's Informal Network:** Respected member, decades of relationships
- **Dock Merchants (honest ones):** Allied against Geist's corruption
- **Merit Council (victim of):** Geist uses Council authority to extort Olric
- No criminal ties - Olric is aggressively honest

## DM Notes

**Plot Hooks:**
- **Direct connection to Kyle's Geist investigation** - Olric is victim and willing to help gather evidence
- Source of quality rope for party (climbing, dungeons, etc.)
- Information from sailors: distant lands, sea monsters, political news
- Ship passage: Olric can arrange affordable, safe transport via trusted captains
- Safe storage for party gear in hidden compartment (if they earn his trust)

**Personality Quirks:**
- Tests rope by hanging his full weight from it before selling
- Tells sea stories while working (most are true)
- Keeps small shrine to Procan (sea god) in shop corner
- Won't sell to anyone who disrespects the sea or sailors
- Remembers every ship he sailed on, every port he visited

**Connection to Campaign:**
- **Geist investigation:** Olric provides victim testimony and evidence of protection racket
- **Mira Saltwind connection:** Family tie to Merchant District, creates cross-tier relationship
- **Information network:** Sailors bring news from across the world
- **Practical supplies:** Party needs rope, climbing gear, waterproof containers for adventures

**Atmosphere Notes:**
- Smell of tar and salt is distinctive, nostalgic for sailors
- Sound of docks creates ambient background (creaking ships, calling gulls, shouting workers)
- Olric works while talking - rope splicing, testing knots, organizing inventory
- Shop feels lived-in, practical - this is a workplace, not a showroom
- Casual racism against Olric (half-elf in human-dominated docks) - he handles it with tired grace

**Evidence Against Geist (Available Here):**
- Olric keeps receipts of all "inspection fines" with dates
- Can testify about threats and harassment
- Knows other shop owners being extorted (can point party to them)
- Has copies of "violation notices" - many are fabricated or trivial
